 ...
 */
public class URLClassDefault {
    //public static String BASE_URL = "http://192.168.1.120:1025/api/v1/shop";
    public static String BASE_URL = AppConfig.BaseUrl + "/api/" + AppConfig.APIVersion + "/default";

    RestAdapter ApiBuilder;
    private static URLClassDefault ourInstance = new URLClassDefault();

    private URLClassDefault() {
    }


    public static URLClassDefault getInstance() {
        if (ourInstance == null) {
            synchronized (URLClassDefault.class) {
                if (ourInstance == null)
                    ourInstance = new URLClassDefault();
            }
        }
        ourInstance.config();
        return ourInstance;
